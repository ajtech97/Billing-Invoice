...
if($paid=="2"){
    $query=mysql_query("select *,DATEDIFF(DueDate,CURDATE()) as diff,GROUP_CONCAT(`EmpId`) AS EmployeeId from invoice_table_data where invoice_del_yes_no='yes' and InvoiceStatus=1 GROUP BY `InvoiceNo` ORDER BY DueDate ASC;");
}
if($paid=="3"){
    $query=mysql_query("select *,GROUP_CONCAT(`EmpId`) AS EmployeeId from invoice_table_data where invoice_del_yes_no='yes' and InvoiceStatus=2 GROUP BY `InvoiceNo` ORDER BY DueDate ASC;");
}
